...
#SBATCH -o /home/kirilb/orcd/pool/TopK/SyntheticGraphRuns/logs/%x_%A_%a.out
#SBATCH -e /home/kirilb/orcd/pool/TopK/SyntheticGraphRuns/logs/%x_%A_%a.err
...
#
# The script will compute the full Cartesian product of
#   (n, p, d, k, seed)
# and then submit itself as a SLURM array.
#
# Each array task runs exactly one combination and saves to
...
PYTHON_SCRIPT="/home/kirilb/orcd/pool/TopK/train_siglip_bipartite_npz.py"
...
# ------------------------------------------------------------
# Submit mode: when run directly with bash, submit this file as
# an array job and stop here.
# ------------------------------------------------------------
if [ -z "${SLURM_ARRAY_TASK_ID:-}" ]; then
  echo "Submitting ${TOTAL_RUNS} runs"
  ...
  sbatch --array=0-$((TOTAL_RUNS - 1))%${MAX_CONCURRENT} "$0"
  exit $?
fi

# ------------------------------------------------------------
# Array-task mode: decode SLURM_ARRAY_TASK_ID into one grid point.
# The order is:
#   n major, then p, then d, then k, then seed minor.
# ------------------------------------------------------------
TASK_ID=${SLURM_ARRAY_TASK_ID}
if [ "$TASK_ID" -ge "$TOTAL_RUNS" ]; then
  echo "Task ${TASK_ID} is outside the grid size ${TOTAL_RUNS}. Exiting."
  exit 0
fi

seed_idx=$(( TASK_ID % NUM_SEED ))
rem=$(( TASK_ID / NUM_SEED ))
